        ...
        private void searchsite()
        {
            if (rad_youtubesearch.Checked == true)
            {
                if (rad_ytvideosec.Checked == true)
                {
                    ...
                    search = search.Replace(" ", "+");
                    setupvid("http://www.youtube.com/results?search_query=" + search, @"(?<=.*translation-title.*>).*(?=</a></h3>)", @"(?<=.*result-item-translation-title.*href=""/watch[?]v=)[a-zA-Z0-9_-]+", @"(?<=.*video-thumb ux-thumb yt-thumb-default-.*//)i.*[.]jpg(?=.*video-time)");
                    VidType = "Youtube";
                }
                else
                {
                    if (rad_ytchannelsec.Checked == true)
                    {
                        ...
                        search = search.Replace("  ", "+");
                        search = search.Replace(" ", "+");
                        setupvid("http://www.youtube.com/results?search_type=search_users&search_query=" + search + "&uni=3", @"(?<=title=.*href=""/user/).*(?=[?].*results_main)", @"(?<=title=.*href="").*(?=[?].*results_main)", @"(?<=thumb.*img src=""//).*[.][a-z]+");
        ...
        private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            switch (e.Argument.ToString())
            {
                ...
                case "YoutubeChannel":
                    ...
                    setupvid(currentytchannel, @"(?<=watch[?]v=.*(\n)    ).*", @"(?<=<a href=""/watch[?]v=)[a-zA-Z0-9-_]+(?=.*title)", @"(?<=src=""//)i[0-9]+.*ytimg.*mqdefault.jpg");
                    break;
